...
def _row(**kwargs) -> dict:
    ...
    row.update(basis="recomputed", methodology="revised (2011)", source_key="ebcnv2021_depenses", source_table="pov_2021.dta")
    row.update(kwargs)
    return row


def recomputed_rows(household: pd.DataFrame | None = None, by_function: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    def emit(frame: pd.DataFrame, **where) -> None:
        w = frame["weight_pop"]
        rows.append(_row(wave=2021, indicator="expenditure_pc_mean", unit="DT per year",
                         value=weighted_mean(frame["expenditure_pc"], w), **where))
        rows.append(_row(wave=2021, indicator="consumption_pc_mean", unit="DT per year",
                         value=weighted_mean(frame["consumption_pc"], w), **where))
        rows.append(_row(wave=2021, indicator="expenditure_hh_mean", unit="DT per year",
                         value=weighted_mean(frame["expenditure_total"], frame["weight_hh"]), **where))
        rows.append(_row(wave=2021, indicator="poverty_rate", unit="percent",
                         value=100 * weighted_mean(frame["poor"].eq("poor"), w), **where))
        rows.append(_row(wave=2021, indicator="extreme_poverty_rate", unit="percent",
                         value=100 * weighted_mean(frame["extreme_poor"].eq("extremely poor"), w), **where))
        rows.append(_row(wave=2021, indicator="gini", unit="index (0-100)",
                         value=gini(frame["expenditure_pc"], w), **where))
        rows.append(_row(wave=2021, indicator="population_share", unit="percent",
                         value=100 * w.sum() / hh["weight_pop"].sum(), **where))
        rows.append(_row(wave=2021, indicator="poor_population", unit="persons",
                         value=float((frame["poor"].eq("poor") * w).sum()), **where))
    ...
    for name, column in [("head education", "head_education"), ("head socio-professional category", "head_csp")]:
        for value, frame in hh.groupby(column, observed=True):
            emit(frame, geography_level="national", geography="Tunisia",
                 subgroup_type=name, subgroup=str(value))
    ...
    for code, dpa in per_function.items():
        common = dict(subgroup_type="COICOP function", subgroup=str(code),
                      source_key="ebcnv2021_depenses", source_table="produit2021_plus.dta")
        rows.append(_row(wave=2021, geography_level="national", geography="Tunisia",
                         indicator="expenditure_pc_by_function", unit="DT per year", value=dpa, **common))
        rows.append(_row(wave=2021, geography_level="national", geography="Tunisia",
                         indicator="budget_share", unit="percent", value=100 * dpa / total, **common))

    return pd.DataFrame.from_records(rows)[COLUMNS]


def build(household: pd.DataFrame | None = None, by_function: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    return panel.sort_values(
        ["indicator", "wave", "geography_level", "geography", "milieu", "subgroup_type", "subgroup", "basis"],
        na_position="first",
